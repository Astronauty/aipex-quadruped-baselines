...
MeasurementMode measurement_mode_from_string(const std::string& mode_str) {
    if (mode_str == "lowlevel_ekf") return MeasurementMode::LOWLEVEL_EKF;
    if (mode_str == "sportmode") return MeasurementMode::SPORTMODE;
    if (mode_str == "mocap") return MeasurementMode::MOCAP;
...
class QuadConvexMPCNode : public rclcpp::Node
{
    public:
        ...
        : Node("quad_convex_mpc_publisher"), count_(0)
        {
            ...
            timer_ = this->create_wall_timer(std::chrono::milliseconds(in1000)), std::bind(&QuadConvexMPCNode::update_mpc_state, this));
    ...
    private:
        ...
        void update_mpc_state()
        {
            ...
            this->convex_mpc.update_x0(
                Vector<double, 13> {
                    theta[0], theta[1], theta[2], // Orientation in roll, pitch, yaw
                    position[0], position[1], position[2], // Position in x, y, z
